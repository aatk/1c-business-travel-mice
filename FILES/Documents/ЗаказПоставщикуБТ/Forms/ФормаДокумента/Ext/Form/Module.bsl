...
Процедура ПодобратьВЗаказУслугиНаСервере(Пар1, Пар2)
	...
	Если Пар1 <> Неопределено Тогда
		...
		Для Каждого МассивУслуг ИЗ Пар1 Цикл
			УправлениеБТВызовСервера.ВключитьУслугуВЗаказПоставщику(МассивУслуг, ВЗаказ);			
		КонецЦикла;				
...
&НаСервере
Процедура ТоварыУслугаПриИзмененииНаСервере(ТекущаяСтрока)
	
	    ТекущиеДанные = Объект.Товары.НайтиПоИдентификатору(ТекущаяСтрока);
		Услуга = ТекущиеДанные.Услуга;
		ТекущиеДанные.Номенклатура = Услуга.НоменклатураБТ.Номенклатура;
		ТекущиеДанные.Количество = 1;
		ТекущиеДанные.Цена = Услуга.Цена;
		ТекущиеДанные.Сумма = Услуга.Сумма;
		ТекущиеДанные.СтавкаНДС = Услуга.СтавкаНДС;		
		ТекущиеДанные.СуммаНДС = Услуга.СуммаНДС;		
		ТекущиеДанные.СуммаСНДС = Услуга.Сумма;		
		ТекущиеДанные.Содержание = Услуга.ПолноеНаименованиеУслуги;		
КонецПроцедуры
...
Процедура ТоварыУслугаПриИзменении(Элемент)
	Если Элементы.Товары.ТекущиеДанные <> Неопределено Тогда
		ТД = Элементы.Товары.ТекущиеДанные.ПолучитьИдентификатор();
		ТоварыУслугаПриИзмененииНаСервере(ТД);
	КонецЕсли;
КонецПроцедуры

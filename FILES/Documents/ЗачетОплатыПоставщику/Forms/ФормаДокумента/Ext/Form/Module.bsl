...
&НаСервере
Функция ПолучитьСуммыЗакупки(ВТЗ)
	//
	
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("МассивУслуг", ВТЗ.ВыгрузитьКолонку("Услуга") );
	Запрос.Текст = "ВЫБРАТЬ
	               |	БалансПоПоставщикамОбороты.Услуга КАК Услуга,
	               |	БалансПоПоставщикамОбороты.СуммаПриход КАК СуммаПродажи
	               |ИЗ
	               |	РегистрНакопления.БалансПоПоставщикам.Обороты(, , , Услуга В (&МассивУслуг)) КАК БалансПоПоставщикамОбороты";
...
Процедура ПодобратьЗаказы(Команда)
	...
	эОтбор = фиксНастройки.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	эОтбор.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("Поставщик");
	эОтбор.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
	эОтбор.ПравоеЗначение = Объект.Контрагент;
	эОтбор.Использование = Истина;
	...
	ПараметрыФормы = Новый Структура("МножественныйВыбор, ФиксированныеНастройки", Истина, фиксНастройки);
	Оп = Новый ОписаниеОповещения("ПодобратьВЗаказУслугиНаСервере", ЭтотОбъект, Неопределено);
	ОткрытьФорму("Документ.ЗаказПоставщикуБТ.ФормаВыбора", ПараметрыФормы,,,,, Оп);
КонецПроцедуры
...
Процедура ОбновитьСуммуРоспределения()
	...
	Для Каждого ДокументИзСписка ИЗ Объект.ПлатежныеДокументы Цикл
		Объект.СуммаДокумента = Объект.СуммаДокумента + ДокументИзСписка.ДокументСписаиняСредств.СуммаДокумента;	
	КонецЦикла;
...
Процедура РаспределитьПоУслугамНаСервере()
	...
	Для Каждого СтрокаСУслугой ИЗ Объект.ПоУслугам Цикл
		...
		Иначе
			СтрокаСУслугой.Сумма = СтрокаСУслугой.СуммаЗакупки;
		КонецЕсли;
...
&НаКлиенте
Процедура ПлатежныеДокументыДокументСписаиняСредствПриИзменении(Элемент)
	ОбновитьСуммуРоспределения();
КонецПроцедуры

...
&НаСервере
Функция ПолучитьJSONДерева(ТС)
	
	ОбъектСервер  =РеквизитФормыВЗначение("Объект");	
	Стр = ОбъектСервер.Политики[ТС].Условие;
...
Функция УстановитьJSONВДерево(Результат, ТС)
	...
	Если ТипЗнч(Результат) = Тип("Структура") Тогда	
		ОбъектСервер = РеквизитФормыВЗначение("Объект");	
		ОбъектСервер.Политики[ТС].Условие = Результат.ДеревоПравил; //Новый ХранилищеЗначения(Результат.ДеревоПравил, Новый СжатиеДанных(9));
		ОбъектСервер.Политики[ТС].ПредставлениеУсловия = Результат.Представление;
...
&НаКлиенте
Процедура ПолитикиПредставлениеУсловияНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	СтандартнаяОбработка = Ложь;
	
	ТС = Элементы.Политики.ТекущиеДанные.ПолучитьИдентификатор();
	
	Оп = Новый ОписаниеОповещения("ВыполнитьПослеЗакрытияОтбора", ЭтотОбъект, ТС);
	
	Пар = Новый Структура("ДеревоПравил");
	Пар.ДеревоПравил = ПолучитьJSONДерева(ТС);
...
&НаКлиенте
Процедура ТаблицаУсловийОтборПриИзменении(Элемент) Экспорт
			
КонецПроцедуры


&НаСервере
Процедура РасчитатьСуммуПриИзмененииНаСервере(ТС)
	
	ТД = Объект.СвязанныеНоменклатуры[ТС];
	ВключаяНДС = Ложь;
	ТД.СуммаНДС	= УправлениеБТВызовСервера.ПолучитьСуммуНДС(ТД.Цена, ТД.СтавкаНДС, ВключаяНДС);
	Если ВключаяНДС Тогда
		ТД.Сумма = ТД.Цена;
	Иначе
		ТД.Сумма = ТД.Цена + ТД.СуммаНДС;
	КонецЕсли;
	
КонецПроцедуры

&НаКлиенте
Процедура РабочиеПолитикиЦенаПриИзменении(Элемент)
	
	Элементы.СвязанныеНоменклатуры.ТекущиеДанные.Сумма = Элементы.СвязанныеНоменклатуры.ТекущиеДанные.Цена; //Пусть будет всегда 18%	
	Если Элементы.СвязанныеНоменклатуры.ТекущиеДанные.СтавкаНДС = ПредопределенноеЗначение("Перечисление.СтавкиНДС.НДС18") Тогда
		Элементы.СвязанныеНоменклатуры.ТекущиеДанные.СуммаНДС = Элементы.СвязанныеНоменклатуры.ТекущиеДанные.Сумма / 118 * 18; //Пусть будет всегда 18%
	Иначе
		Элементы.СвязанныеНоменклатуры.ТекущиеДанные.СуммаНДС = 0;
	КонецЕсли;
...
Функция ДобавитьСтавкуНДС(Договор)
	Возврат Справочники.СтавкиНДСБТ.БезНДС;//Перечисления.СтавкиНДС.БезНДС;//Договор.СтавкаНДС;
КонецФункции

&НаКлиенте
Процедура СвязанныеНоменклатурыПриНачалеРедактирования(Элемент, НоваяСтрока, Копирование)
	
	Если НоваяСтрока Тогда
		ТД = ЭтаФорма.Элементы.СвязанныеНоменклатуры.ТекущиеДанные;
		Если ТД <> Неопределено Тогда 
			ТД.СтавкаНДС = ДобавитьСтавкуНДС(Объект.Договор);
		КонецЕсли;
	КонецЕсли;
...
&НаКлиенте
Процедура РабочиеПолитикиСтавкаНДСПриИзменении(Элемент)
...
&НаКлиенте
Процедура ВидимостьЭкспертногоРежима()
	
	Элементы.СвязанныеНоменклатурыУпрощенныйРежим.Видимость = НЕ Элементы.СвязанныеНоменклатурыЭкспертныйРежим.Видимость;
	Элементы.СвязанныеНоменклатурыОбработчик.Видимость = НЕ Элементы.СвязанныеНоменклатурыЭкспертныйРежим.Видимость;
	
	Элементы.СвязанныеНоменклатурыГруппа5.Видимость = НЕ Элементы.СвязанныеНоменклатурыЭкспертныйРежим.Видимость;
	Элементы.СвязанныеНоменклатурыПрименяетсяК.Видимость = НЕ Элементы.СвязанныеНоменклатурыЭкспертныйРежим.Видимость;
		
	Элементы.СвязанныеНоменклатурыГруппа2.Видимость = НЕ Элементы.СвязанныеНоменклатурыЭкспертныйРежим.Видимость;
	Элементы.РабочиеПолитикиЦена.Видимость = НЕ Элементы.СвязанныеНоменклатурыЭкспертныйРежим.Видимость;
	Элементы.РабочиеПолитикиСумма.Видимость = НЕ Элементы.СвязанныеНоменклатурыЭкспертныйРежим.Видимость;
	
	Элементы.СвязанныеНоменклатурыГруппа3.Видимость = НЕ Элементы.СвязанныеНоменклатурыЭкспертныйРежим.Видимость;
	Элементы.РабочиеПолитикиСтавкаНДС.Видимость = НЕ Элементы.СвязанныеНоменклатурыЭкспертныйРежим.Видимость;
	Элементы.РабочиеПолитикиСуммаНДС.Видимость = НЕ Элементы.СвязанныеНоменклатурыЭкспертныйРежим.Видимость;
		
	Элементы.СвязанныеНоменклатурыРеквизитУстановкиЦены.Видимость = НЕ Элементы.СвязанныеНоменклатурыЭкспертныйРежим.Видимость;
	Элементы.РабочиеПолитикиПолитика.Видимость = НЕ Элементы.СвязанныеНоменклатурыЭкспертныйРежим.Видимость;
	 
КонецПроцедуры

&НаКлиенте
Процедура ЭкспертныйРежим(Команда)
	Элементы.СвязанныеНоменклатурыЭкспертныйРежим.Видимость = Ложь;
	ВидимостьЭкспертногоРежима();
КонецПроцедуры

&НаКлиенте
Процедура УпрощенныйРежим(Команда)
	Элементы.СвязанныеНоменклатурыЭкспертныйРежим.Видимость = Истина;
	ВидимостьЭкспертногоРежима();
КонецПроцедуры
...
Процедура УстановитьЗначениеШаблона(СруктураЗначениеJSON, РабочийИдентификатор)
	...
	ТекущаяСтрока = Объект.СвязанныеНоменклатуры.НайтиПоИдентификатору(РабочийИдентификатор);
	Если ЗначениеЗаполнено(СруктураЗначениеJSON) Тогда
		...
		ТекущаяСтрока.СвязаннаяНоменклатура = СруктураЗначениеJSON.УсловиеДляФормированияУслуги;
		
		//ТекущаяСтрока.СвязаннаяНоменклатура = СруктураЗначениеJSON.СтарыеПараметры.СвязаннаяНоменклатура;
		...
		ТекущаяСтрока.Сумма = СруктураЗначениеJSON.СтарыеПараметры.Сумма;
...
&НаКлиенте
Процедура СвязанныеНоменклатурыПредставлениеСвязаннойУслугиНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	СтандартнаяОбработка = Ложь;
	...
	ПараметрыОкна.Вставить("ВПроцентах", ТД.ЦенаВПроцентах);
	
	СтарыеПараметры = Новый Структура();
	СтарыеПараметры.Вставить("СвязаннаяНоменклатура", ТД.СвязаннаяНоменклатура);
	СтарыеПараметры.Вставить("ПрименяетсяК", ТД.ПрименяетсяК);
	СтарыеПараметры.Вставить("ЦенаВПроцентах", ТД.ЦенаВПроцентах);
	...
	СтарыеПараметры.Вставить("Сумма", ТД.Сумма);
	СтарыеПараметры.Вставить("РеквизитУстановкиЦены", ТД.РеквизитУстановкиЦены);

	ПараметрыОкна.Вставить("ТД", СтарыеПараметры);
	
	Если ТипЗнч(ТД.СвязаннаяНоменклатура) = Тип("СправочникСсылка.ЭлементОтбора") Тогда
		ПараметрыОкна.Вставить("УсловиеДляФормированияУслуги", ТД.СвязаннаяНоменклатура);
	КонецЕсли;
...
&НаКлиенте
Процедура СвязанныеНоменклатурыПредставлениеСвязаннойУслугиОчистка(Элемент, СтандартнаяОбработка)
	ТД = ЭтаФорма.Элементы.СвязанныеНоменклатуры.ТекущиеДанные;
	РабочийИдентификатор = ТД.ПолучитьИдентификатор();

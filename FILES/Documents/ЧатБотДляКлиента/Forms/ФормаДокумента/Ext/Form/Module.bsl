...
Функция НайтиПредшественника(Элемент, Схема)
	НайденныеПредшественники = Новый Массив; УсловияПредшественника = "";
	Для Каждого ТекЭлемент Из Схема Цикл 
		Если ТипЗнч(ТекЭлемент) = Тип("ЭлементГрафическойСхемыСоединительнаяЛиния") ИЛИ
			ТипЗнч(ТекЭлемент) = Тип("ЭлементГрафическойСхемыДекоративнаяЛиния") ИЛИ
			ТипЗнч(ТекЭлемент) = Тип("ЭлементГрафическойСхемыДекорация") 
				Тогда
            Если ТекЭлемент.КонецЭлемент = Элемент Тогда
				НайденныеПредшественники.Добавить(ТекЭлемент.НачалоЭлемент);
				Если ТипЗнч(ТекЭлемент.НачалоЭлемент) = Тип("ЭлементГрафическойСхемыУсловие") Тогда
					УсловияПредшественника = ТекЭлемент.Наименование;
				ИначеЕсли ТипЗнч(ТекЭлемент.НачалоЭлемент) = Тип("ЭлементГрафическойСхемыВыборВарианта") Тогда
					УсловияПредшественника = ТекЭлемент.НачалоВариант.Наименование;
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
	Если НайденныеПредшественники.Количество() = 1 Тогда 
		Возврат Новый Структура("Предшественник, УсловияПредшественника", УправлениеБТВызовСервера.md5(НайденныеПредшественники[0].Имя), УсловияПредшественника)
	Иначе
		Возврат Новый Структура("Предшественник, УсловияПредшественника", "", "")
	КонецЕсли;
КонецФункции

&НаСервереБезКонтекста
Функция НайтиСледующий(Элемент, Схема)
	//НайденныеПредшественники = Новый Массив;
	НайденныеСледующие = Новый Массив;
	Для Каждого ТекЭлемент Из Схема Цикл 
		Если ТипЗнч(ТекЭлемент) = Тип("ЭлементГрафическойСхемыСоединительнаяЛиния") ИЛИ
			ТипЗнч(ТекЭлемент) = Тип("ЭлементГрафическойСхемыДекоративнаяЛиния") ИЛИ
			ТипЗнч(ТекЭлемент) = Тип("ЭлементГрафическойСхемыДекорация") 
			...
			Если ТекЭлемент.НачалоЭлемент = Элемент Тогда
				НайденныеСледующие.Добавить(ТекЭлемент.КонецЭлемент);
				НайденныеПредшественникиСлед = НайтиПредшественника(ТекЭлемент.КонецЭлемент, Схема);
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
	Если НайденныеПредшественникиСлед = Неопределено Тогда
		Возврат ""
	ИначеЕсли НайденныеПредшественникиСлед.Предшественник = "" //для след блока много предшесвенников
		И НайденныеСледующие.Количество() > 0 Тогда 
		Возврат УправлениеБТВызовСервера.md5(НайденныеСледующие[0].Имя)
	Иначе
		Возврат ""
	КонецЕсли;
КонецФункции
...
&НаСервереБезКонтекста
Функция ПолучитьТипЗапросаСхемы(ЭлементСхемы)
	
	Если ТипЗнч(ЭлементСхемы) = Тип("ЭлементГрафическойСхемыВложенныйБизнесПроцесс") Тогда
		Значение = Перечисления.ТипЗапросаСхемы.ВложенныйПроцесс;
	Иначе
		Значение = Перечисления.ТипЗапросаСхемы[СтрЗаменить(СтрЗаменить(""+ТипЗнч(ЭлементСхемы),"Элемент графической схемы ","")," ", "")];
	//ИначеЕсли ТипЗнч(ЭлементСхемы) = Тип("ЭлементГрафическойСхемыВыборВарианта") Тогда
	//	Значение = Перечисления.ТипЗапросаСхемы.ВыборВарианта;
	//ИначеЕсли ТипЗнч(ЭлементСхемы) = Тип("ЭлементГрафическойСхемыДействие") Тогда
	//	Значение = Перечисления.ТипЗапросаСхемы.Действие;	
	//ИначеЕсли ТипЗнч(ЭлементСхемы) = Тип("ЭлементГрафическойСхемыЗавершение") Тогда
	//	Значение = Перечисления.ТипЗапросаСхемы.Завершение;
	//ИначеЕсли ТипЗнч(ЭлементСхемы) = Тип("ЭлементГрафическойСхемыОбработка") Тогда
	//	Значение = Перечисления.ТипЗапросаСхемы.Обработка;
	//ИначеЕсли ТипЗнч(ЭлементСхемы) = Тип("ЭлементГрафическойСхемыРазделение") Тогда
	//	Значение = Перечисления.ТипЗапросаСхемы.Разделение;
	//ИначеЕсли ТипЗнч(ЭлементСхемы) = Тип("ЭлементГрафическойСхемыСлияние") Тогда
	//	Значение = Перечисления.ТипЗапросаСхемы.Слияние;
	//ИначеЕсли ТипЗнч(ЭлементСхемы) = Тип("ЭлементГрафическойСхемыСтарт") Тогда
	//	Значение = Перечисления.ТипЗапросаСхемы.Старт;
	//ИначеЕсли ТипЗнч(ЭлементСхемы) = Тип("ЭлементГрафическойСхемыУсловие") Тогда
	//	Значение = Перечисления.ТипЗапросаСхемы.Условие;
	КонецЕсли;
...
&НаСервере
Функция РазобратьСхему()
	
	Схема = Новый ГрафическаяСхема;
	ДДСхемы = Base64Значение( Объект.СхемаВзаимодействия );
	ИмяФайла = ПолучитьИмяВременногоФайла("json");
	ДДСхемы.Записать(ИмяФайла);
	Схема.Прочитать(ИмяФайла);
	УдалитьФайлы(ИмяФайла);
	
	//
	Выгрузка = Объект.Последовательность.Выгрузить();
	Объект.Последовательность.Очистить();
	Для Каждого ЭлементСхемы ИЗ Схема.ЭлементыГрафическойСхемы Цикл
		//
		Если ЭлементСхемы <> Неопределено Тогда
			Если ТипЗнч(ЭлементСхемы) = Тип("ЭлементГрафическойСхемыСоединительнаяЛиния") ИЛИ
				ТипЗнч(ЭлементСхемы) = Тип("ЭлементГрафическойСхемыДекоративнаяЛиния") ИЛИ
				ТипЗнч(ЭлементСхемы) = Тип("ЭлементГрафическойСхемыДекорация") Тогда
				//				
			Иначе
				
				НовыйЭтап = Объект.Последовательность.Добавить();
				НовыйЭтап.Идентификатор = УправлениеБТВызовСервера.md5(ЭлементСхемы.Имя);   //Строка(Новый УникальныйИдентификатор())
				НовыйЭтап.ТипЗапроса = ПолучитьТипЗапросаСхемы(ЭлементСхемы);
				
				НовыйЭтап.Представление = ЭлементСхемы.Имя;
				НовыйЭтап.СопровождающийТекст = ЭлементСхемы.Наименование;
				
				НайденныйПредшественник = НайтиПредшественника(ЭлементСхемы, Схема.ЭлементыГрафическойСхемы);
				
				НовыйЭтап.Предшественник 	= НайденныйПредшественник.Предшественник;
				НовыйЭтап.Следующий 		= НайтиСледующий(ЭлементСхемы, Схема.ЭлементыГрафическойСхемы);
				
				НовыйЭтап.УсловиеПредшественника = НайденныйПредшественник.УсловияПредшественника;
				
				СтараяСтрока = Выгрузка.Найти(НовыйЭтап.Идентификатор, "Идентификатор");
				Если СтараяСтрока <> Неопределено Тогда
					НовыйЭтап.СервисШины = СтараяСтрока.СервисШины;
					НовыйЭтап.РасширенноеУсловие = СтараяСтрока.РасширенноеУсловие;
					НовыйЭтап.РасширенныйСопровождающийТекст = СтараяСтрока.РасширенныйСопровождающийТекст;					
				КонецЕсли;
...
&НаКлиенте
Процедура ВыполнитьПослеЗакрытияСхемы(Результат, Параметр) Экспорт
	
	Если ТипЗнч(Результат) = Тип("Структура") Тогда
		ДДСхемы = "";
		Результат.Свойство("Схема", ДДСхемы); 
		Объект.СхемаВзаимодействия = Base64Строка(ДДСхемы);
		//Результат.Свойство("Схема", Объект.СхемаВзаимодействия);
...
Процедура ОткрытьСхему(Команда)
	...
	Оп = Новый ОписаниеОповещения("ВыполнитьПослеЗакрытияСхемы", ЭтотОбъект);
	
	ДДСхемы = Base64Значение( Объект.СхемаВзаимодействия );
	Пар = Новый Структура("Схема", ДДСхемы);

	ОткрытьФорму("ОбщаяФорма.ФормаСхемы", Пар,,,,,Оп);

КонецПроцедуры

&НаСервере
Процедура ТестСкриптаНаСервере()
	Дерево = ЧатБотБТСервер.СформироватьДеревоВопросовПоДокументу(Объект.Ссылка);
	ТекстJSON = УправлениеБТВызовСервера.ВJSON(Дерево);
	ТестовыйРезультат = УправлениеБТВызовСервера.ДобавитьТабыВJSON(ТекстJSON);
КонецПроцедуры

&НаКлиенте
Процедура ТестСкрипта(Команда)
	ТестСкриптаНаСервере();

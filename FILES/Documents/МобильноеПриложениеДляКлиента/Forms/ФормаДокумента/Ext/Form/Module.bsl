...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Объект.ДоступныеСтраницы.Количество() = 0 Тогда
		ЗаполнитьЭкраны();
	КонецЕсли;
...
//Код в форме
Функция ЗаполнитьЭкраны()
	СписокЭкранов = МобильноеПриложениеБТСервер.ПолучитьСписокЭкранов();
	Для Каждого Экран ИЗ СписокЭкранов Цикл
		НоваяСтраница = Объект.ДоступныеСтраницы.Добавить();
		ЗаполнитьЗначенияСвойств(НоваяСтраница, Экран);
		НоваяСтраница.Включено = Истина;
	КонецЦикла;
КонецФункции

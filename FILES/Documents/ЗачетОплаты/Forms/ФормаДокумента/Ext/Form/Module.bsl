...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	Если НЕ УправляемыеФормыКлиент.ПередЗакрытием(ЭтаФорма, Отказ, ЗавершениеРаботы, ТекстПредупреждения,СтандартнаяОбработка, "Вместо") Тогда
		Если НЕ ЗавершениеРаботы Тогда
			Если Объект.СуммаДокумента > Объект.ПоУслугам.Итог("Сумма") Тогда
				
				//Режим = РежимДиалогаВопрос.ДаНет;
				//Оповещение = Новый ОписаниеОповещения("ПослеЗакрытияВопроса", ЭтаФорма, Параметры);
				//ПоказатьВопрос(Оповещение, "Обнаружена не распределенная сумма!!! " + "Продолжить закрытие?", Режим, 0);	
				ПоказатьПредупреждение(,"Обнаружена не распределенная сумма!!!", 10, "Ошибка распределение сумм");	
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
...
&НаСервере
Функция ПолучитьСуммыПродажи(ВТЗ)
	//
	
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("МассивУслуг", ВТЗ.ВыгрузитьКолонку("Услуга") );
	Запрос.Текст = "ВЫБРАТЬ
	               |	БалансПоПродажамУслугИТоварамБТОбороты.Услуга КАК Услуга,
	               |	БалансПоПродажамУслугИТоварамБТОбороты.СуммаПриход КАК СуммаПродажи
	               |ИЗ
	               |	РегистрНакопления.БалансПоПродажамУслугИТоварамБТ.Обороты(, , , Услуга В (&МассивУслуг)) КАК БалансПоПродажамУслугИТоварамБТОбороты";
...
&НаСервере
Процедура ПодобратьВЗаказУслугиНаСервере(МассивУслугИЗаказов, Пар2)
	
	ВТЗ = Новый ТаблицаЗначений;
	ВТЗ.Колонки.Добавить("Услуга");
	ВТЗ.Колонки.Добавить("СуммаПродажи");
	
	Если МассивУслугИЗаказов <> Неопределено Тогда
		Для Каждого УслугаЗаказ ИЗ МассивУслугИЗаказов Цикл
			Если ТипЗнч(УслугаЗаказ) = Тип("ДокументСсылка.Услуга") Тогда
				НоваяСтрока = ВТЗ.Добавить();
				НоваяСтрока.Услуга = УслугаЗаказ;
			Иначе
				Для Каждого УслугаВЗаказе ИЗ УслугаЗаказ.Товары Цикл
					НоваяСтрока = ВТЗ.Добавить();
					НоваяСтрока.Услуга = УслугаВЗаказе.Услуга;					
				КонецЦикла;
			КонецЕсли;
		КонецЦикла;				
	...
	//ВТЗ
	//Объект.ПоУслугам.Добавить();
	ТЗ = ПолучитьСуммыПродажи(ВТЗ);
	Объект.ПоУслугам.Загрузить(ТЗ);
...
Процедура ПодобратьЗаказы(Команда)
	...
	ПараметрыФормы = Новый Структура("МножественныйВыбор, ФиксированныеНастройки", Истина, фиксНастройки);
	Оп = Новый ОписаниеОповещения("ПодобратьВЗаказУслугиНаСервере", ЭтотОбъект, Неопределено);
	ОткрытьФорму("Документ.ЗаказКлиентаБТ.ФормаВыбора", ПараметрыФормы,,,,, Оп);
КонецПроцедуры

&НаСервере
Процедура ОбновитьСуммуРоспределения()
	Объект.СуммаДокумента = 0;
	Для Каждого ДокументИзСписка ИЗ Объект.ПлатежныеДокументы Цикл
		Объект.СуммаДокумента = Объект.СуммаДокумента + ДокументИзСписка.ДокументПоступленияСредств.СуммаДокумента;	
	КонецЦикла;

КонецПроцедуры

&НаСервере
Процедура ДокументПоступленияСредствПриИзмененииНаСервере()
	Объект.ПлатежныеДокументы.Очистить();	
	НоваяСтрока = Объект.ПлатежныеДокументы.Добавить();
	НоваяСтрока.ДокументПоступленияСредств = Объект.ДокументПоступленияСредств;

	ОбновитьСуммуРоспределения();
КонецПроцедуры


&НаКлиенте
Процедура ДокументПоступленияСредствПриИзменении(Элемент)
	ДокументПоступленияСредствПриИзмененииНаСервере();
КонецПроцедуры
...
Процедура РаспределитьПоУслугамНаСервере()
	...
	ОстатокСуммы = Объект.СуммаДокумента;
	Для Каждого СтрокаСУслугой ИЗ Объект.ПоУслугам Цикл
		Если СтрокаСУслугой.Услуга.СуммаКлиента > ОстатокСуммы Тогда
			СтрокаСУслугой.Сумма = ОстатокСуммы;
		ИначеЕсли ОстатокСуммы = 0 Тогда
			СтрокаСУслугой.Сумма = 0;
		Иначе
			СтрокаСУслугой.Сумма = СтрокаСУслугой.СуммаПродажи;
		КонецЕсли;
		
		ОстатокСуммы = ОстатокСуммы - СтрокаСУслугой.Сумма;
	КонецЦикла;
...
&НаКлиенте
Процедура ПлатежныеДокументыДокументПоступленияСредствПриИзменении(Элемент)
	ОбновитьСуммуРоспределения();
КонецПроцедуры

...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ УправляемыеФормыСервер.ПриСозданииНаСервере(ЭтаФорма, Отказ, СтандартнаяОбработка, "Вместо") Тогда
		...
		ЗадачиПоСогласованию.Параметры.УстановитьЗначениеПараметра("СогласованиеКомандировки", Объект.Ссылка);
	КонецЕсли;
...
&НаСервере
Процедура ОтобразитьСхему()
	
	Если ЗначениеЗаполнено(Объект.СхемаСогласования) Тогда
		СхемаСтрокой = Объект.СхемаСогласования.СхемаВзаимодействия;
		
		СхемаЖСОН = Base64Значение( СхемаСтрокой );
...
&НаКлиенте
Процедура СхемаСогласованияПриИзменении(Элемент)
...
&НаСервере
Процедура ЗапуститьПроцессНаСервере()
	Если Объект.Ссылка <> Документы.СогласованиеКомандировки.ПустаяСсылка() Тогда 
		УправлениеБТСервер.СоздатьСледующуюЗадачуСогласования(Объект.Ссылка);
	Иначе

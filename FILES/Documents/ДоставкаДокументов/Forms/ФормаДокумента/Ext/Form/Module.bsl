...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ ЗначениеЗаполнено(Объект.СтатусДоставки) Тогда
		Объект.СтатусДоставки = Перечисления.СтатусыДоставки.Подготовлен;
	КонецЕсли;
...
&НаКлиенте
Процедура ЗаполнитьЗаказыКлиента(Команда)
	...
	эОтбор = фиксНастройки.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	эОтбор.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("СтатусДоставки");
	эОтбор.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
	эОтбор.ПравоеЗначение = ПредопределенноеЗначение("Перечисление.СтатусыДоставки.ПустаяСсылка");
	эОтбор.Использование = Истина;	 
	эОтбор.РежимОтображения = РежимОтображенияЭлементаНастройкиКомпоновкиДанных.Недоступный;

	ПараметрыФормы = Новый Структура("РежимВыбора, МножественныйВыбор, ФиксированныеНастройки", Истина, Истина, фиксНастройки); //
	Оп = Новый ОписаниеОповещения("ПодобратьВЗаказУслугиНаСервере", ЭтотОбъект, Неопределено);
	ОткрытьФорму("Документ.ЗаказКлиентаБТ.ФормаСписка", ПараметрыФормы,,,,, Оп);
КонецПроцедуры

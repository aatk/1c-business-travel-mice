...
Процедура ЗаполнитьВалюты()
	...
	ЗагружаемыеВалюты = Новый Массив;
...
Процедура ЗаполнитьДанныеСтрокиТаблицыНаОсновеВалюты(СтрокаТаблицы, Валюта);
	...
	ДанныеКурса = Новый Структура("ДатаКурса, Курс, Кратность", ТекущаяДата(), 1, 1);
...
Функция ВыполнитьЗагрузкуКурсов()
	...
	Возврат Неопределено; //ДлительныеОперации.ВыполнитьВФоне(РегламентноеЗадание.Метаданные.ИмяМетода, ПараметрыЗагрузки, ПараметрыВыполнения);
